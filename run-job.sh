...
experiment_name=$2
option=$3
$RESOURCE_GROUP_NAME = subri-ai-rg
$WORKSPACE = subri-ai-ml
...
if [[ -z "$experiment_name" ]]; then
  if [[ "$job" =~ pipeline.yml ]]; then
    run_id=$(az ml job create -f "$job" --resource-group "subri-ai-rg" --workspace-name "subri-ai-ml"  --query name -o tsv --set settings.force_rerun=True)
  else
    run_id=$(az ml job create -f "$job" --resource-group "subri-ai-rg" --workspace-name "subri-ai-ml"  --query name -o tsv)
  fi
else
  run_id=$(az ml job create -f "$job" --resource-group "subri-ai-rg" --workspace-name "subri-ai-ml" --query name -o tsv --set experiment_name="$experiment_name" --set settings.force_rerun=True)
fi
